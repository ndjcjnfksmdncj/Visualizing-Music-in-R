...
#' @export
plot.av_fft <- function(x, y, dark = TRUE, legend = TRUE, keep.par = FALSE, useRaster = FALSE, vline = NULL, ...){
  if(!isTRUE(keep.par)){
  ...
  # Add x-axis
  axis(1, at=seq(0, max(attr(x, 'time')), 1), labels=seq(0, max(attr(x, 'time')), 1))
  # Add the y-axis, starting at 1, and remove the 0th value
  axis(2, at = seq(round(min(log(attr(x, 'frequency'))[-1])), round(max(log(attr(x, 'frequency'))[-1])), by = 1))
